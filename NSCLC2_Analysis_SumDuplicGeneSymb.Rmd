...
merged_counts_filtered_forClustering_gene_symbol <- merged_counts_filtered_forClustering %>%
  EnsemblID_to_GeneSymbol(., aggregation_method = "sum") # As a main difference from the other analysis, here we sum duplicated GENE SYMBOL 
...
write.table(TPM_forBIODICA, "/home/francesco.massaini/Projects/BIODICA/data/TPM_NSCLC2_Biodica_sumDuplGenes.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
```
...
metasample_df <- read.table("/home/francesco.massaini/Projects/BIODICA/work/TPM_NSCLC2_Biodica_sumDuplGenes_ICA/TPM_NSCLC2_Biodica_sumDuplGenes_ica_A.xls", header = TRUE, row.names = 1)
metagene_df <- read.table("/home/francesco.massaini/Projects/BIODICA/work/TPM_NSCLC2_Biodica_sumDuplGenes_ICA/TPM_NSCLC2_Biodica_sumDuplGenes_ica_S.xls", header = TRUE, row.names = 1)
...
clinical = rownames_to_column(clinical, var = "Sample")

# Join clinical data with the long format data
combined_data <- as.data.frame(left_join(long_metasample, clinical, by = "Sample"))

# Generate violin plots for each IC for each clinical variable
clinical_vars = colnames(clinical)[-1]  # List of clinical variables
ICs = unique(combined_data$IC)

# Aggregate violin plot for each clinical variable
for (clin_var in clinical_vars) {
  tmp_clinical = clinical[, c("Sample", clin_var)]
  colnames(tmp_clinical) = c("Sample", "clin")
  merged <- left_join(long_metasample, tmp_clinical, by = "Sample")
...
for (clin_var in clinical_vars) {
  for (IC_value in ICs){
    df = combined_data %>%
      ...
      dplyr::select(Value, all_of(clin_var))
    colnames(df) = c("Value", "clin")
    
    test_result <- aov(Value ~ clin, data = df)
    ...
    if (!is.na(p_value) && p_value < p_threshold) {  # Check p-value
      p <- ggplot(df, aes(x = clin, y = Value, fill = clin)) +
            geom_violin(trim = FALSE) +
            geom_jitter(width = 0.2, alpha = 0.5, size = 1.5, color = "black") +
            labs(title = paste("Significant Violin plot of", clin_var, "for", IC_value), y = "IC Level", x = clin_var, fill = clin_var) +
            theme_minimal() +
            theme(axis.text.x = element_text(angle = 45, hjust = 1))
      print(p)
    }
