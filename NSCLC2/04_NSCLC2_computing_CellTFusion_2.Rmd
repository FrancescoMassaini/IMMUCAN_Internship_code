...
deconv = read.delim("../../IMMUCAN_data/NSCLC2/03_Deconvolution/NSCLC2_deconvolution_fromCellTFusion.txt", sep = "\t", check.names = FALSE)
...
counts.norm = read.delim("../../IMMUCAN_data/NSCLC2/02_TPM/NSCLC2_TPM_forDeconv.txt", row.names = 1, check.names = FALSE) 
...
```{r}
network = compute.WTCNA(tfs, corr_mod = 0.7, clustering.method = "ward.D2") # Soft_Threshold file is in usr_projects/Results
...
save(features_sig, file = "../../../Desktop/IMMUCAN_data/NSCLC2/04_CellTFusion/CellTFusion_modules_deconv_NSCLC2.RData")
cell_dendrograms = identify.cell.groups(dt[[1]], network[[1]], features_sig, height = 20) 
```

8. Cell groups analysis 
```{r}
res = cell.groups.analysis(dt[[1]], traitData, cell_dendrograms, cut.height = 0.7, stat.test = 'Anova', pval = 0.05, height = 20)
...
```{r}
prediction_anova = compute.ML.models(res[[1]], traitData, partition = 0.7, k_folds = 5, n_rep = 20, n_top = 3, n_iterations = 20, file_name = "Tutorial", return_top = T, seed = 123)
```

```{r}
prediction_Boruta = compute.ML.models(res_all[[1]], traitData, partition = 0.8, k_folds = 10, n_rep = 10, n_top = 2, n_iterations = 10, file_name = "Tutorial_Boruta", return_top = T, seed = 123, boruta = T, rough_fix = T, tentative = F)
```
