...
# Il risultato di questo processo è un dataframe dove la prima riga del file originale diventa l'insieme dei nomi delle colonne, e i valori nella prima colonna del file diventano i nomi delle righe del dataframe, facilitando l'accesso e la manipolazione dei dati in analisi successive.

traitData = read.csv("/home/francesco.massaini/Desktop/IMMUCAN_data/NSCLC2/01_Clinical_Data/Daniel/NSCLC2_for_R_update_2024.csv") %>%
  column_to_rownames(var = "immucan_id") # %>%
  # mutate(across(c(average_number_of_cigarettes, number_of_years),
  #               ~ na_if(.x, "U")))
...
colnames(counts.norm) = str_replace_all(colnames(counts.norm), "\\.", "-")
colnames(counts.norm) <- str_remove_all(string = colnames(counts.norm), pattern = "-RNA-01|-RNA-02|-RNA-04|-FIXT-01")
counts.norm = counts.norm[,common_patients]

imagingIMC = read.csv("/home/francesco.massaini/Desktop/IMMUCAN_data/NSCLC2/01_IF_IMC/IMC_cell_type_densities.csv", row.names = 1)
common_patients = intersect(rownames(imagingIMC), rownames(deconv))
...
#####
coldata_categorical = traitData %>%
  dplyr::select(where(is.character) | where(is.factor))

# Inizializza una lista per mantenere i nomi delle colonne problematiche
problematic_columns = list()

# Cicla attraverso le colonne e verifica il numero di livelli unici
for (col in names(coldata_categorical)) {
  if (is.factor(coldata_categorical[[col]])) {
    # Controlla i fattori direttamente
    num_levels = nlevels(coldata_categorical[[col]])
  } else {
    # Per i caratteri, conta i livelli unici
    num_levels = length(unique(coldata_categorical[[col]]))
  }
  
  # Stampa il nome della colonna se ha meno di 2 livelli unici
  if (num_levels < 2) {
    cat("La colonna", col, "ha solo", num_levels, "livello/i unico/i.\n")
    problematic_columns[[col]] = num_levels
  }
}

# Se la lista è vuota, nessun problema trovato
if (length(problematic_columns) == 0) {
  cat("Nessuna colonna problematica trovata.\n")
} else {
  print(problematic_columns)
}

traitData = select(traitData, -names(problematic_columns))
```
...
save(dt, file = "CellTFusion_Subgroups.RData")
