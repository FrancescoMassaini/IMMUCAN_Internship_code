...
# 02 Analizing TPM
```{r Distribution}
TPM = load("../IMMUCAN_data/BC2/02_TPM/BC2_estimated_TPM.RData")
colSums(gexp_tpm) 
...
ggplot2::ggplot(TPM_long, aes(x = log1p(TPM)))+
  ...
  labs(title = "log(TPM+1) Distribution", 
       ...
       y = "Density")
```
# 03 BC2 Filtering Samples and saving file
```{r Filtering samples}
# Filtering samples with the list samples_list
samples_list = read.delim("../IMMUCAN_data/BC2/Patient_to_include_from_rna_samples_bc2_to_create_custom_cohort.tsv")

BC2_samples <- intersect(colnames(TPM), samples_list$sample)
BC2 <- TPM[,BC2_samples] 

write.csv(BC2, file = "../IMMUCAN_data/BC2/02_TPM/BC2_fromAndreaRdata_filtered_Samples.csv")
```
...
Compute_Samples_dendrogram(BC2)
Compute_Samples_Heatmap(BC2)
