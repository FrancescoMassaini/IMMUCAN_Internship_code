...
BC2_forDeconv = TPM
BC2_forDeconv = EnsemblID_to_GeneSymbol(raw_counts_file = BC2_forDeconv, aggregation_method = "mean")
write.csv(BC2_forDeconv, file = "/home/francesco.massaini/Desktop/IMMUCAN_data/BC2/02_TPM/BC2_fromAndreaRdata_filtered_Samples.csv")
```
...
# Working only on top varible genes
```{r }
TPM_filtered_forClustering = TPM %>%
  filter_low_expr_genes(., avg_expr_threshold = 1, zero_count_percent_threshold = 0.9)
...
# Fisher's tests on clinical varibles
...
fixed_clinical_feature = "PCR"
clinical_features = colnames(clinical)[colnames(clinical) != fixed_clinical_feature]

for (feat in clinical_features){
  selected = dplyr::select(clinical, fixed_clinical_feature , feat)
  contingency_table = table(selected)
  chi_result <- chisq.test(as.matrix(contingency_table))
    if (chi_result$p.value < 0.05) {
      print(paste(feat, chi_result$p.value))
    ...
    cat(feat, "No Significance\n")
    }
}

specific_contincency = table(dplyr::select(clinical, PCR , lymphocyte_infiltration))
mosaicplot(specific_contincency + 0.1,
                 main = "Mosaic plot",
                 sub = paste("P-value:", format(chi_result$p.value, digits = 2)),
                 color = TRUE
                )
